...
* **ISO strings come in following forms: `'YYYY-MM-DD'`, `'HH:MM:SS.ffffff[±<offset>]'`, or both separated by an
...
* **The way `'aggregate()'` and `'transform()'` find out whether the passed function accepts an element or the whole
  Series is by passing it a single value at first and if it raises an error, then they pass it the whole Series.**
...
```python
cdef
enum < enum_name >: < member_name_1 >, < member_name_2 >, ...
```
